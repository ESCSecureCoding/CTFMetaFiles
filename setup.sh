...
docker run --detach --name nginx-proxy-letsencrypt --network http_proxy --volumes-from nginx-proxy --volume /var/run/docker.sock:/var/run/docker.sock:ro --env "DEFAULT_EMAIL=fabian.hick@exxeta.com" jrcs/letsencrypt-nginx-proxy-companion
